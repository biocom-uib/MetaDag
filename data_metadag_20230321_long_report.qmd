...
# Similarities for Direct (MSA) method
...
colorsK <- list(Kingdom= c("Animals"="red","Plants"="green","Fungi"="black","Protists"="yellow"))
...
colorsP=list(Class=col)
names(colorsP$Class)=nombres
...
colores <- list(Kingdom= c("Animals"="red","Plants"="green","Fungi"="black","Protists"="yellow"))
...
cooordinates=as_tibble(mds7$points)
colnames(cooordinates)=paste("Component",1:7)
ggpairs(cooordinates,columns=1:4,aes(color=meta_taxo$Kingdom,alpha=0.5,title="MDS 4 dimensions projection",legend=1),upper=list(continuous="points")) +scale_fill_manual(values = colores$Kingdom)+ theme(legend.position = "left")
