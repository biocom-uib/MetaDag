...
```{r ,warning=FALSE}
readh_mDAG=function(x) {DAG=read.graph(file=x,
                                      format="graphml")
return(DAG)}
...
big_MBB=function(org){
  ...
  x=x[(!is.na(x))]
  x=x[x!="NA"]
  tt=data.frame(sort(table(x),decreasing=TRUE))
  return(tt)
}


big_MBB_list_raw= lapply(Organism,FUN=function(x) big_MBB(x))
names(big_MBB_list_raw)=Organism
nMBB=max(sapply(big_MBB_list_raw,FUN=function(x) length(x)))
nMBB

big_MBB_list=lapply(big_MBB_list_raw,
                    FUN=function(x){
