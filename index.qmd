# MetaDag Eukaryotes experiment {.unnumbered}
...
knitr::kable(data.frame(Directory_files_and_folders=dir(path_exp),
...
```{r}
knitr::kable(MBB[1:20,1:30]) %>%   scroll_box(width = "100%", height = "200px")
```
...
dimnames(reverse_reactions)$reverse_reactions=c("One direction","Reverse reaction")
reverse_reactions
```
...
```{r}
graph_mDAG=read.graph(paste0(path_exp,"Individuals/hsa/hsa_mDAG.graphml"),format = "graphml")
summary(graph_mDAG)
...
knitr::include_graphics(
  paste0(path_exp,"Global/core/core_mDAG.svg"))
```
...
```{r fig.cap="Core mDAG is empty"}
knitr::include_graphics(
  paste0(path_exp,"Global/core/core_RC.svg"))
```
...
graph_pan_mDAG=read.graph(
  paste0(path_exp,"TaxonomyLevels/Kingdom/Animals/pan/Animals_pan_mDAG.graphml"), format = "graphml")
...
graph_pan_RC=read.graph(
  paste0(path_exp,"TaxonomyLevels/Kingdom/Animals/pan/Animals_pan_RC.graphml"), format = "graphml")
...
knitr::include_graphics(paste0(path_exp,
                               "Individuals/hsa/hsa_mDAG_biggerDAG.svg"))
```
...
meta_taxo = meta_taxo %>% left_join(Freq_Phylum) %>% left_join(Freq_Class)
meta_taxo = meta_taxo %>%  arrange(Kingdom,desc(Freq_Phylum),Phylum,desc(Freq_Class),Class)
head(meta_taxo)
```
...
anot <- HeatmapAnnotation(df=dff, col = colorsK,show_legend = TRUE)

#S=Sim_MSA_mDAG

MSA_heat_1 <- Heatmap(matrix = Sim_MSA_mDAG, 
                          column_title=
                          "m-DAGs MSA-similarity \n Eukaryotes by Kingdom",
            name = "MSA Similarity",
             heatmap_legend_param = list(title="Similality",at = seq(0,1,by=0.1)),
        col=rev(viridis(256)),
        cluster_rows = FALSE,
...
namesP=unique(dff$Phylum)
col=rainbow(length(nombres))
colorsP=list(Phylum=col)
names(colorsP$Phylum)=namesP
anotation_MSA_H2 <- HeatmapAnnotation(df=dff, col = colorsP,show_legend = FALSE)
MSA_heat_2 <- Heatmap(matrix =
                      ...
                      cluster_rows = FALSE,
                      cluster_columns = FALSE,
                      top_annotation =anotation_MSA_H2,
                      show_column_names = FALSE,
                      show_row_names = FALSE,
...
anotation_heat <- HeatmapAnnotation(df=dff, col = colorsK)
dim(dff)
...
Mun_heat_1<- Heatmap(matrix = Sim_Mun_mDAG, 
    ...
    at = seq(0,1,by=0.1)),
        ...
        cluster_rows = FALSE,
        cluster_columns = FALSE,
        top_annotation = anotation_heat,
        show_column_names = FALSE, 
        show_row_names = FALSE,
        left_annotation = rowAnnotation(df = dff, col = colorsK))
...
aux_order=table(meta_animals$Phylum)
dff<-meta_taxo %>% filter(Kingdom=="Animals") %>% select(Phylum) %>% as.data.frame()
names_phylum=unique(dff$Phylum)
names_phylum
col=rainbow(length(nombres))
colorsP=list(Phylum=col)
names(colorsP$Phylum)=names_phylum
anotation <- HeatmapAnnotation(df=dff, col = colorsP)
...
Mun_heat_2 <- Heatmap(matrix = Sim_Mun_mDAG[1:nrow(dff),1:nrow(dff)], 
    ...
    at = seq(0,1,by=0.1)),
        ...
        cluster_rows = FALSE,
        cluster_columns = FALSE,
        top_annotation = anotation,
        show_column_names = FALSE, 
        show_row_names = FALSE,
        left_annotation = rowAnnotation(df = dff,
                                        col = colorsP,                                        show_annotation_name=FALSE))
...
# Similarity comparisons Eukaryotes
...
aux$mDag=names(aux)
aux=aux %>% pivot_longer(cols=`0576`:`0286`,
...
aux$mDag=names(aux)
aux=aux %>% pivot_longer(cols=`0576`:`0286`,names_to="mDag_2",values_to="Sim_Mun")
...
```{r cache=TRUE}
ggpairs(Sim_comp[,c(3,5)])
```

```{r}
boxplot(Sim_comp[,c(3,5)])
```
...
p1<- ggplot(data=big_MBB_list2) + 
  geom_line(mapping=aes(x=index,y=MBBsize,group = Organism,color=Kingdom),na.rm=TRUE) + 
...
p0<-ggplot(data=aux2) + 
  geom_line(mapping=aes(x=index,y=csize,group = Organism,color=Kingdom),na.rm=TRUE) + 
  scale_x_continuous(trans='log10') + 
  scale_y_continuous(trans='identity') +
  scale_color_manual(values =COLOR_KINGDOM[colors_kingdom$Kingdom])+
...
p2<- ggplot(data=aux2) + 
  ...
  scale_y_continuous(trans="identity") +
  ylim(0,1039)+
   ggtitle("Plot  of size  weak components decreasing index.")+  ylab("Weak components size") + xlab("Index")+
  scale_color_manual(values =COLOR_KINGDOM[colors_kingdom$Kingdom])
...
p3<- ggplot(data=data2) + 
  ...
  scale_y_continuous(trans="identity") +
  ylim(0,25)+
   ggtitle("Plot  of size  weak components  decreasing index 2 to 20.")+  ylab("Weak components size") + xlab("Index")+
  scale_color_manual(values =COLOR_KINGDOM[colors_kingdom$Kingdom])
